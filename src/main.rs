    ...
    match prog {
        ...
        Prog::Lt(p1, p2) => {
            ...
            match (p1_type, p2_type) {
                ...
                _ => Err(
                    "Il y a une comparaison qui ne fait pas intervenir deux entiers".to_string(),
                ),
            }
        }
        Prog::If(p, q, r) => {
...
fn reduce(prog: &Prog) -> Option<Prog> {
    match prog {
        Prog::Add(p1, p2) => match (p1.as_ref(), p2.as_ref()) {
        ...
        Prog::If(cond, p, q) => match &**cond {
            ...
            _ => {
                ...
                Some(Prog::If(
                    Box::new(reduced_cond),
                    Box::new(*p.clone()),
                    Box::new(*q.clone()),
                ))
            }
        },
        Prog::Lt(p1, p2) => match (p1.as_ref(), p2.as_ref()) {
